...
def get_peer_list_db():
	query="select * from peer_list;"
	x.execute(query)
	record = x.fetchall()
	for row in record:
		if row[2] not in peer_list_dict:
			peer_list_dict[row[2]]=row[0]
			peer_ip=row[1]
			asn_data=get_asn_info(peer_ip)
			x.execute('update peer_list set peer_lat=%s,peer_lon=%s where peer_id=%s',(float(str(asn_data['lat'])),float(str(asn_data['lon'])),int(row[2])))
			conn.commit()
			time.sleep(1)
